  ...
  return html`
    <div class="chat-area">
      <div class="chat-header">
        ...
        <div class="chat-title">${activeSessionId ? '🏃 Workout' : '🏃 ActiveLog.ai'}</div>
        <div class="actions">
          ...
          ${loadingSession ? html`<div class="empty-state"><span class="spinner" style="font-size:1.5rem;width:24px;height:24px"></span><div class="empty-hint" style="margin-top:.75rem">Loading campaign...</div></div>` :
            messages.length === 0 ? html`
              <div class="empty-state">
                ...
                <div class="empty-title">Let's get moving.</div>
                <div class="empty-hint">Log a workout, plan your training, or get fitness advice.</div>
                <div class="empty-prompts">
                  <button class="prompt-chip" onclick=${() => setInput('I just finished a 5k run in 28 minutes')}>💪 Log a workout</button>
                  <button class="prompt-chip" onclick=${() => setInput('Create a training plan for my first 10k')}>🏃 Training plan</button>
                  <button class="prompt-chip" onclick=${() => setInput('What should I eat after a strength workout?')}>🍎 Nutrition advice</button>
                  <button class="prompt-chip" onclick=${() => setInput('Show me my progress this week')}>📊 Check progress</button>
                </div>
              </div>
            ` :
